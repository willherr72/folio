...
- [x] Write failing behavior tests for rotated drawing, cancel/end and page routing/scroll.
- [x] Extend PageView with draw tool, draft path, pointer capture, safe cancellation and optional page activation/interaction disabling.
- [x] Implement continuous/single viewport with lazy full-page rendering, scroll-selected page, explicit navigation requests and anchored Ctrl+wheel zoom.
- [x] Validate targeted tests/build and commit; report interface and evidence.
...
- [x] Write failing preference/focus/modal behavior tests.
- [x] Provide validated persistent settings hook and reusable accessible centered modal.
- [x] Build settings UI and consistent dark/light styles; keep PDF paper colors intact.
- [x] Adapt signature pad to shared modal; validate tests and commit.
...
- [x] Add failing app tests for modal cancellation, settings, drawing and page identity.
- [x] Remove engine badge, integrate asynchronous modal actions/close, block background shortcuts, wire page-scoped state/history callbacks, settings and Draw toolbar/properties.
- [x] Run full frontend/native checks and browser/native smoke, review, resolve substantive findings.
- [x] Merge local branch, package versioned 0.2 update, verify and update launcher; commit handoff.
