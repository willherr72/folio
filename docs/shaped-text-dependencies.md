...
Folio vendors the exact HarfRust 0.13.3 package with a two-file resource-status patch. `GlyphBuffer::is_successful()` exposes interruption, and optional shaping limits constrain intermediate glyph storage and instrumented lookup operations. Folio rejects unsuccessful buffers before reading glyph results. The caller requests at most 16,384 initialized slots per backing array, including intermediate expansion, and supplies at most 262,144 instrumented operations per run / 6,291,456 across a maximum-size line. Defaults preserve upstream behavior for callers that do not select limits. Before bounds or outline calculation, a shared TrueType preflight also limits expanded composite depth to 16 levels and aggregate work to 100,000 component nodes, simple points and contours across distinct glyphs. Bounds are cached per glyph; malformed nonempty outlines cannot silently become whitespace. These are specific algorithmic guards, not a hard process-memory cap or CPU deadline. Exact provenance, the complete patch, hostile fixtures and measured checks are documented in [shaping resource limits](shaping-resource-limits.md). This resolves observable partial-shaping acceptance; independent PDF reader compatibility remains a separate production gate.
