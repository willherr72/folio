...
As of v0.11.0, [verified single positioned runs](existing-text-editing.md) support bounded same-font spacing and affine transforms. [Issue15 status](issue15-status.md) separates this completed stage from adjacent groups and nested forms. Current shaped-text support and remaining reader gates are recorded in [issue14 status](issue14-status.md).
