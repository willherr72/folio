"""Check adjacent-group exports against independently authored PDF references.

Generate fixtures with the native text_group tests. Comparisons are exact;
...
parser.add_argument('--input', type=Path, default=Path('artifacts/text-group-native'))
parser.add_argument('--output', type=Path, default=Path('artifacts/text-group-native/reader-results.json'))
...
for output in exports:
    ...
    row['passed'] = all(p[k] for p in row['pages'] for k in ('mupdfExactText', 'mupdfExactRaster', 'pypdfExactText'))
    results.append(row)
