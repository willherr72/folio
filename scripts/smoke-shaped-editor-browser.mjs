...
const input = path.join(root, "artifacts/shaped-text/editor-native");
const output = path.join(root, "artifacts/shaped-text/editor-browser");
const stems = (await readdir(input)).filter(name => name.endsWith(".prepared.json")).map(name => name.replace(/\.prepared\.json$/, "")).sort();
expect(stems).toHaveLength(32);
expect(stems).toEqual(["ligatures-on", "ligatures-off", "composed", "decomposed", "marks", "arabic", "indic", "supplementary"]
...
try {
  ...
  for (const stem of stems) for (const zoom of [150, 300]) {
    const record = JSON.parse(await readFile(path.join(input, `${stem}.prepared.json`), "utf8"));
    const native = record.result.preview;
    await page.goto(`${origin}/tests/fixtures/shaped-editor.html?case=${encodeURIComponent(stem)}&zoom=${zoom}`);
    ...
    cases.push({ stem, zoom, rotation: record.overlay.rotation, text: native.text, fontId: native.fontId, ...result });
