...
const original=resolve(run,'Positioned practice.pdf'),saved=resolve(run,'Positioned edited.pdf');copyFileSync(resolve('examples/Positioned text demo.pdf'),original);const originalHash=hash(original);
...
try{
 await open(original);
 for(const [from,to] of [['Rotated words','Rotated new words'],['Skewed words','Skewed new words'],['Spaced words','Spaced new words'],['Positioned words','Positioned new words']]){
  await edit(from,to);await page.getByRole('button',{name:'Undo',exact:true}).click();await expect.poll(text).toContain(from);await page.getByRole('button',{name:'Redo',exact:true}).click();await expect.poll(text).toContain(to);checks.push(from+' native replacement and undo/redo');
 }
 await page.getByRole('button',{name:'Find',exact:true}).click();await page.getByRole('searchbox',{name:'Find in document'}).fill('Positioned new words');await expect(page.locator('.search-count')).toContainText('1 of 1');await page.getByRole('button',{name:'Close search',exact:true}).click();checks.push('edited positioned source searchable');
 ...
 await page.getByRole('button',{name:'Close Positioned practice.pdf',exact:true}).click();renameSync(original,resolve(run,'Original moved.pdf'));await open(saved);await expect.poll(text).toContain('Positioned new words');checks.push('native save/reopen with original moved and unchanged');
 const editStarted=Date.now();await edit('Spaced new words','Recovered spaced words');
 ...
 await page.screenshot({path:resolve(run,'positioned-edited.png')});await browser.close();
 ...
 await expect(page.getByRole('dialog',{name:'Restore your workspace?'})).toBeVisible();await page.getByRole('button',{name:'Restore workspace',exact:true}).click();await expect.poll(text).toContain('Recovered spaced words');checks.push('crash recovery restores unsaved positioned source with both original and saved paths unavailable');
 await page.screenshot({path:resolve(run,'positioned-recovered.png')});writeFileSync(resolve(run,'results.json'),JSON.stringify({passed:true,binarySha256:state.sha256,originalSha256:originalHash,checks},null,2));console.log('PASS',checks);
